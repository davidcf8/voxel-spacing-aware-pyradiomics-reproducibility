...
static char ngtdm_docstring[] = "Arguments: Image, Mask, Distances, Ng, force2D, force2Ddimension, kernelRadius (opt), Voxels (opt)";
static char ngtdm_spacing_docstring[] = "Arguments: Image, Mask, Distances, Ng, force2D, force2Ddimension, kernelRadius (opt), Voxels (opt), Spacing (opt).";
...
static PyObject *cmatrices_calculate_ngtdm_spacing(PyObject *self, PyObject *args);
...
static PyMethodDef module_methods[] = {
  ...
  { "calculate_ngtdm_spacing", cmatrices_calculate_ngtdm_spacing, METH_VARARGS, ngtdm_spacing_docstring },
...
static PyObject *cmatrices_calculate_ngtdm_spacing(PyObject *self, PyObject *args)
{
  int Ng, force2D, force2Ddimension, kernelRadius = 1;
  PyObject *image_obj, *mask_obj, *distances_obj, *voxelcoords_obj = NULL, *spacing_obj = NULL;
  PyArrayObject *image_arr, *mask_arr, *voxels_arr = NULL, *spacing_arr = NULL;
  int Nd, Na, Nvox = 1;
  int *size, *bb, *strides;
  npy_intp dims[3];
  PyArrayObject *ngtdm_arr, *angles_arr;
  int *image, *angles, *voxels = NULL;
  double *spacing = NULL;
  char *mask;
  double *ngtdm;
  int v;

  if (!PyArg_ParseTuple(args, "OOOiii|iOO",
                        &image_obj, &mask_obj, &distances_obj,
                        &Ng, &force2D, &force2Ddimension,
                        &kernelRadius, &voxelcoords_obj, &spacing_obj))
  {
    PyErr_SetString(PyExc_ValueError, "Invalid arguments passed to calculate_ngtdm");
    return NULL;
  }
  ...
  if (try_parse_voxels_arr(voxelcoords_obj, &voxels_arr, Nd, &Nvox, kernelRadius) < 0)
  {
    Py_XDECREF(image_arr);
    Py_XDECREF(mask_arr);
    free(size);
  ...
  if (spacing_obj && spacing_obj != Py_None)
  {
    spacing_arr = (PyArrayObject *)PyArray_FROM_OTF(spacing_obj, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    if (!spacing_arr)
    {
      PyErr_SetString(PyExc_ValueError, "Could not convert spacing to numpy array");
      goto fail;
    }

    if (PyArray_NDIM(spacing_arr) != 1 || PyArray_DIM(spacing_arr, 0) != Nd)
    {
      PyErr_SetString(PyExc_ValueError, "spacing must be a 1D array of length Nd");
      goto fail;
    }
  ...
  if ((size_t)(dims[0] * dims[1] * dims[2]) > SIZE_MAX)
  {
    PyErr_SetString(PyExc_RuntimeError, "Too many elements in NGTDM output array.");
    goto fail;
  }
  ...
  {
    set_bb(v, bb, size, voxels, Nd, Nvox, kernelRadius, force2Ddimension);
    if (!calculate_ngtdm_spacing(image, mask, size, bb, strides, angles, Na, Nd,
                          ngtdm + v * Ng * 3, Ng, spacing))
    {
      PyErr_SetString(PyExc_IndexError, "Calculation of NGTDM failed.");
      free(bb);
      Py_XDECREF(ngtdm_arr);
