...
class RadiomicsGLSZM(base.RadiomicsFeaturesBase):
  ...
  def _calculateMatrix(self, voxelCoordinates=None):
    ...
    if finiteVolumeVoxelSpacing:
      if self.voxelBased or voxelCoordinates is not None:
        raise NotImplementedError('Finite-volume voxelSpacing GLSZM is not implemented for voxel-based extraction')
      ...
      self.logger.debug(
        'Applying finite-volume zero-order-hold GLSZM expansion with spacing %s, repeat factors %s, '
      ...
      if numpy.max(numpy.abs(finiteVolumeGeometry['relativeSpacingError'])) > 0.10:
        self.logger.warning(
          'GLSZM voxelSpacing finite-volume representation has relative spacing error %s',
          finiteVolumeGeometry['relativeSpacingError']
        )
      if finiteVolumeGeometry['expansionFactor'] > 8:
        self.logger.warning(
          'GLSZM voxelSpacing finite-volume expansion factor %d expands image shape from %s to %s',
  ...
  def _calculateCoefficients(self):
    ...
    if self.settings.get('weightingNorm', None) == 'voxelSpacing':
      self.logger.debug(
        'GLSZM voxelSpacing mode preserves standard zone topology formulas; anisotropic inputs are represented '
        'on a finite-volume zero-order-hold lattice before matrix construction'
      )
