...
class RadiomicsGLRLM(base.RadiomicsFeaturesBase):
  ...
  def _calculateMatrix(self, voxelCoordinates=None):
    ...
    pixelSpacing = self.inputImage.GetSpacing()[::-1]  # Z, Y, X
    finiteVolumeVoxelSpacing = (
      self.weightingNorm == 'voxelSpacing'
      and not spacing_utils.is_isotropic_spacing(pixelSpacing)
    )

    if finiteVolumeVoxelSpacing:
      if self.voxelBased or voxelCoordinates is not None:
        raise NotImplementedError('Finite-volume voxelSpacing GLRLM is not implemented for voxel-based extraction')

      repeatFactors = spacing_utils.compute_finite_volume_repeat_factors(
        pixelSpacing, force2D, force2Ddimension
      )
      finiteVolumeGeometry = spacing_utils.compute_finite_volume_geometry(
        pixelSpacing, repeatFactors, force2D, force2Ddimension, self.imageArray.shape
      )
      self.logger.debug(
        'Applying finite-volume zero-order-hold GLRLM expansion with spacing %s, repeat factors %s, '
        'represented spacing %s, relative spacing error %s and expanded shape %s',
        pixelSpacing,
        repeatFactors,
        finiteVolumeGeometry['representedSpacing'],
        finiteVolumeGeometry['relativeSpacingError'],
        finiteVolumeGeometry['expandedShape']
      )
      if numpy.max(numpy.abs(finiteVolumeGeometry['relativeSpacingError'])) > 0.10:
        self.logger.warning(
          'GLRLM voxelSpacing finite-volume representation has relative spacing error %s',
          finiteVolumeGeometry['relativeSpacingError']
        )
      if finiteVolumeGeometry['expansionFactor'] > 8:
        self.logger.warning(
          'GLRLM voxelSpacing finite-volume expansion factor %d expands image shape from %s to %s',
          finiteVolumeGeometry['expansionFactor'],
          self.imageArray.shape,
          finiteVolumeGeometry['expandedShape']
        )
      matrixImage = spacing_utils.expand_array_zero_order_hold(self.imageArray, repeatFactors)
      matrixMask = spacing_utils.expand_array_zero_order_hold(self.maskArray, repeatFactors)
    else:
      matrixImage = self.imageArray
      matrixMask = self.maskArray

    Nr = numpy.max(matrixImage.shape)
    ...
    # Optionally apply a weighting factor. The voxelSpacing mode uses the
    # finite-volume matrix above for anisotropic spacing, and reduces exactly
    # to standard unweighted GLRLM for isotropic spacing.
    if finiteVolumeVoxelSpacing:
      self.logger.debug('voxelSpacing GLRLM represented by finite-volume expansion; no angular weighting applied')
    elif self.weightingNorm == 'voxelSpacing' and spacing_utils.is_isotropic_spacing(pixelSpacing):
      self.logger.debug('voxelSpacing requested on isotropic spacing; using standard unweighted directional handling')
      ...
      P_glrlm = numpy.sum(P_glrlm * weights[None, None, None, :], axis=3, keepdims=True)
